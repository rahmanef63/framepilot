...
`yagni:` casing/naming drift — lib file `dataPrompt.ts` (camelCase) vs component dir `dataprompt/` (lowercase); routes mix Indonesian (beranda/panduan/proyek) with English (editor/template). Pick one per axis. [src/lib/dataPrompt.ts]
